"""
DB security — harden `get_latest_submission_for_owner` (SECURITY DEFINER).

Why:
    The teacher "latest submission detail" endpoint relies on a SECURITY DEFINER
    helper that reads from `learning_submissions` (student-scoped RLS).
    Because the function executes with elevated privileges, it must minimize
    its attack surface:
...
@pytest.mark.anyio
async def test_latest_submission_owner_helper_is_hardened() -> None:
    _require_db_or_skip()
    try:
    ...
    with psycopg.connect(_dsn()) as conn:  # type: ignore[arg-type]
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                select count(*)
                 ...
                 where rp.specific_schema = 'public'
                   and rp.routine_name = 'get_latest_submission_for_owner'
            ...
            assert "teacher_id = p_owner_sub" not in definition
            assert re.search(
