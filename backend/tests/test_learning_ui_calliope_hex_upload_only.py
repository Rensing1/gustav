"""
SSR UI — Calliope tasks are HEX upload-only (MVP)

Intent:
    Ensure `Task.kind="calliope"` is rendered as an upload-only task in the
    student UI (no text mode, no textarea) and accepts `.hex`.
"""
...
async def _prepare_calliope_fixture() -> tuple[str, str, str]:
    """Create a course/unit with one released calliope task and return (student_sid, course_id, unit_id)."""
    ...
    async with (await _client()) as c:
        ...
        _ = await c.post(
            ...
            json={"instruction_md": "### Calliope Aufgabe", "criteria": ["K1"], "calliope": {}},
        )
...
@pytest.mark.anyio
async def test_ui_renders_calliope_tasks_as_hex_upload_only() -> None:
    sid, course_id, unit_id = await _prepare_calliope_fixture()
    async with (await _client()) as c:
    ...
    # Calliope tasks: no mode switch + no textarea
    ...
    assert 'accept=".hex,application/x.makecode.hex"' in html
    assert 'data-allowed-mime="application/x.makecode.hex"' in html
