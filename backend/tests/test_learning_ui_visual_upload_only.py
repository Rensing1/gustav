"""
SSR UI — Visual tasks are upload-only (Phase 3)

Intent:
    Ensure `Task.kind="visual"` is rendered as an upload-only task in the
    student UI (no text mode, no textarea).
"""
...
async def _prepare_visual_fixture() -> tuple[str, str, str]:
    """Create a course/unit with one released visual task and return (student_sid, course_id, unit_id)."""
    ...
    async with (await _client()) as c:
        ...
        _ = await c.post(
            ...
            json={"instruction_md": "### Visual Aufgabe", "criteria": ["K1"], "visual": {}},
        )
...
@pytest.mark.anyio
async def test_ui_renders_visual_tasks_as_upload_only():
    sid, course_id, unit_id = await _prepare_visual_fixture()
    async with (await _client()) as c:
    ...
    # Visual tasks: no mode switch + no textarea
