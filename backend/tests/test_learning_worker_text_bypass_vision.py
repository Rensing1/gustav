...
@pytest.mark.anyio
async def test_worker_bypasses_vision_for_text_and_preserves_text() -> None:
    ...
    submission = usecase.execute(
        CreateSubmissionInput(
            ...
            size_bytes=None,
            sha256=None,
            idempotency_key="bypass-vision-text",
        )
    )
    ...
    processed = run_once(dsn=worker_dsn, vision_adapter=exploding, feedback_adapter=feedback)
