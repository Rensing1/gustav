...
def test_teacher_course_ai_usage_date_filter_uses_berlin_midnight() -> None:
    assert app_routes._parse_usage_filter_timestamp("2026-08-08") == datetime(
        2026, 8, 7, 22, 0, tzinfo=timezone.utc
    )
    assert app_routes._parse_usage_filter_timestamp("2026-03-29") == datetime(
...
async def test_teacher_course_ai_usage_rejects_invalid_pagination(
    monkeypatch: pytest.MonkeyPatch,
    query: str,
) -> None:
    headers = _mock_bearer_auth(monkeypatch, sub="teacher-usage-validation", roles=["teacher"], name="Ada")

    async with httpx.AsyncClient(transport=ASGITransport(app=main.app), base_url="http://test") as client:
        response = await client.get(
            f"/api/teaching/views/courses/00000000-0000-0000-0000-000000000000/ai-usage?{query}",
            headers=headers,
        )
...
@pytest.mark.anyio
async def test_teacher_course_ai_usage_aggregates_grouped_rows_and_filters_learner_list(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    store = install_session_store(monkeypatch, main)
    monkeypatch.setattr(
        ...
        lambda subs: {"student-usage-a": "Lena", "student-usage-b": "Noah"},
    )
    teacher = store.create(sub="teacher-usage-filled", roles=["teacher"], name="Ada", ttl_seconds=60)
    student_a = store.create(sub="student-usage-a", roles=["student"], name="Lena", ttl_seconds=60)
    student_b = store.create(sub="student-usage-b", roles=["student"], name="Noah", ttl_seconds=60)
    headers = _mock_bearer_auth(monkeypatch, sub=teacher.sub, roles=["teacher"], name="Ada")

    monkeypatch.setattr(
        app_routes,
        "_list_teacher_course_ai_usage_events",
        lambda **_kwargs: [
            {
                "student_sub": student_a.sub,
    ...
    async with httpx.AsyncClient(transport=ASGITransport(app=main.app), base_url="http://test") as client:
        ...
        response = await client.get(
            f"/api/teaching/views/courses/{course_id}/ai-usage?student_sub={student_a.sub}",
            headers=headers,
        )

    assert response.status_code == 200
    body = response.json()
    assert body["course"]["members_count"] == 2
    assert [item["student"]["sub"] for item in body["learners"]] == [student_a.sub]
    assert body["totals"]["input_tokens"] == 20
    assert body["totals"]["output_tokens"] == 7
    assert body["totals"]["total_tokens"] == 27
    assert body["totals"]["known_events"] == 2
    assert body["totals"]["unknown_events"] == 1
    assert [item["stage"] for item in body["totals"]["breakdown"]] == ["feedback", "reply"]
