...
This module restores a legacy Supabase dump into a dedicated schema and
...
def ensure_app_login_role(dry_run: bool) -> PhaseResult:
    ...
    try:
        ...
        return PhaseResult(
            name="app_login_role",
            status="failed",
            errors=[f"Invalid DB_PORT value: {db_port!r} (expected integer)."],
            details={"app_user": app_user, "action": "invalid_db_port"},
        )

    try:
        with psycopg.connect(  # type: ignore[call-arg]
            host=db_host,
            port=port,
            user=db_superuser,
            ...
            with conn.cursor() as cur:  # type: ignore[attr-defined]
                cur.execute("select 1 from pg_roles where rolname = 'gustav_limited'")
                if cur.fetchone() is None:
                    return PhaseResult(
                        name="app_login_role",
                        status="failed",
                        errors=[
                            "Role 'gustav_limited' not found. Ensure migrations have been applied (e.g. `supabase db reset`)."
                        ],
                        details={"app_user": app_user, "action": "role_missing"},
                    )
                ...
                if role_exists:
                    ...
                    action = "created"
                cur.execute(sql.SQL("grant gustav_limited to {}").format(sql.Identifier(app_user)))
    except Exception as exc:
        return PhaseResult(
            name="app_login_role",
            status="failed",
            errors=["Failed to provision app DB login role.", str(exc)],
            details={"app_user": app_user, "action": "provision_failed"},
        )
    ...
    if ready:
        return PhaseResult(
            name="app_login_role",
            status="success",
            inserted=1 if action == "created" else 0,
            details={"app_user": app_user, "action": action},
        )
