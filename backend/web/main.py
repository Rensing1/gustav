...
def _extract_makecode_files_from_evidence_v1(evidence_md: str) -> dict[str, str]:
    """Extract selected MakeCode file contents from `makecode.evidence.v1` Markdown.

    Notes:
        We intentionally parse only the tiny subset of Markdown we generate
        ourselves in `backend/makecode/hex_evidence_v1.py`:
        - headings: `### file: <json-string>`
        - fenced blocks: ```lang ... ```

        This keeps the renderer deterministic and avoids adding Markdown parsing
        dependencies into the SSR layer.
    """
...
def _compact_makecode_evidence_v1_for_display(evidence_md: str) -> str:
    """Return compact Markdown for learner/teacher UI from a full MakeCode evidence report."""
...
def _render_submission_text_html(*, text_src: str) -> str:
    """Render submission text/evidence Markdown to safe HTML with scoped wrappers.

    This is used in both learner history and teacher live detail so Evidence
    formatting stays consistent across views.
    """
    raw = str(text_src or "")
...
def _render_submission_text_container(
    ...
    *,
    ...
    text_html = _render_submission_text_html(text_src=text_src)
