...
Lehrer-Startseite und Lerneinheitenkatalog erhalten ihren Datenbankzugang über `create_app(teacher_catalog_providers=...)`. `UnitCatalogService` in `backend/teaching/services/unit_catalog.py` bündelt Status-, Such-, Sortier- und Kurszuordnungsregeln ohne Kenntnis von FastAPI. Beide synchronen Handler verwenden dieselben eigentümergebundenen Repository-Operationen; die Startseite übernimmt die ersten drei Katalogeinträge und liest ihre Kursliste nur einmal. Kurszuordnungen und Abschnittszähler/Aktivitätsdaten werden in `repo_catalog_queries.py` gebündelt gelesen. Der Katalog benötigt höchstens vier fachliche Leseabfragen, jeweils mit transaktionslokalem RLS-Kontext. Bestehende 200er-Listenlimits und deren Sortierung bleiben erhalten; Abschnittsinhalte werden für Zähler nicht mehr vollständig geladen.
...
Der interne Upload-Proxy erhält Transport und Telemetrie über `create_app(learning_upload_proxy_providers=...)`. Sein typisierter Provider verwendet den bestehenden asynchronen HTTP-Transport und die unveränderte, gegen Loggerfehler abgesicherte Protokollierung aus `learning_upload_proxy.py`. Anmeldung, Same-Origin-, Zieladress-, Header-, MIME- und Größenprüfungen bleiben im HTTP-Adapter; erst danach wird weitergeleitet. Der Proxy löst Transport und Telemetrie nicht mehr über die Learning-Fassade oder Endpoint-Globals auf. Seine gemeinsamen Authentifizierungs-/Konfigurationshelfer bleiben separate Folgearbeit; die vorhandene Proxy-Konfiguration ändert sich nicht. Die Upload-Freigaben sind inzwischen einschließlich DB- und Speicherzugang entkoppelt, wie im folgenden Absatz beschrieben.

Die Erstellung von Upload-Freigaben erhält Datenbank und Speicher über `create_app(learning_upload_intent_providers=...)`. Der Handler löst den DB-Adapter nach Rollen-, CSRF- und Eingabeprüfung einmal auf; der bestehende `ListSubmissionsUseCase` und die anschließende Aufgabentyp-Abfrage verwenden denselben Adapter. Beide Operationen behalten ihre DB-Sichtbarkeitsprüfungen. Der Speicher wird erst nach erfolgreicher Autorisierung und MIME-Prüfung aufgelöst. `backend/storage/supabase_factory.py` baut den bereits für Materialabrufe verwendeten Storage3-Adapter aus der Standardkonfiguration. Erfolgreicher Aufbau wird pro Provider wiederverwendet, Fehler bleiben wiederholbar und liefern private Fehlerantworten. Der Upload-Intent-Endpunkt benötigt keine Learning-Fassade mehr; Authentifizierungs- und Konfigurationshelfer werden direkt verwendet. Repository-Aufbau, Abfragen und Signierung laufen im begrenzten Threadpool. Die globale Speicherverdrahtung verbleibender Abgabe-/Teaching-Verbraucher ist hiervon getrennt und noch nicht abgelöst.

Die sechs Dialog-Endpunkte erhalten ihre Dienste über `create_app(learning_dialog_providers=...)`. Nur der zustandslose DB-Adapter wird wiederverwendet; der bestehende KI-Generator mit seinem Verbrauchspuffer entsteht weiterhin pro Anfrage neu. Die synchronen Handler laufen im begrenzten Threadpool. Der globale Dialog-Testschalter entfällt; übrige gemeinsame Authentifizierungshelfer werden separat migriert. Die echte DB-Charakterisierung dokumentiert mit TD-010 zwei Bestandsgrenzen: nicht übersetzte Sichtbarkeitsfehler beim Start sowie weiterhin lesbare eigene bestehende Dialoge nach Entzug von Freigabe oder Mitgliedschaft. Dies ist keine Sicherheitsfreigabe.

`backend/learning/submission_payload.py` enthält die reinen Abgabe-Eingaberegeln mit übergebenem Upload-Limit. `backend/learning/workers/submission_job_persistence.py` trennt Kontextabfragen und Ergebnis-/Retry-Metadaten von der Worker-Orchestrierung. Der Aufrufer besitzt weiterhin Verbindung, RLS-Kontext und Transaktionsgrenzen; Persistenzfunktionen rufen keine KI-Adapter auf. Weitere Aufteilung der umfangreichen Verarbeitungszweige und Queue-/Retry-Steuerung ist separate Folgearbeit.

Der H5P-Dienst registriert unter `h5p-service/routes/` getrennte Bereiche für `authoring`, `player` und `ajax`. Startup liefert die benötigten Editor-/Player-/Storage-/Transportkontexte. Öffentliche Assets liegen weiterhin vor der Authentifizierung; die drei Bereiche werden erst nach Authentifizierung, Rollen- und Review-Middleware registriert. Die eigene Ergebnisweiterleitung bleibt vor Lumis Auffangrouter, die Fehlerbehandlung danach. HTML, Gestaltung, Cachezeiten und Sicherheitsregeln werden bei dieser internen Trennung nicht verändert.
