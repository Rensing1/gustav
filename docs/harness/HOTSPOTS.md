...
- `backend/web/routes/teaching.py` ist dadurch von 6146 auf 790 LOC gesunken. C1 ist damit als Route-Handler-Monolith weitgehend abgebaut; offen bleiben die schrittweise Ablösung der Teaching-Fassade, Repo-Provider-Globals, Storage-Adapter-Globals und gemeinsam genutzte Live-Helfer in fokussierte Provider-/Read-Model-Module.
- `backend/teaching/repo_row_mappers.py` übernimmt seit Closeout v1.1 reine Material-/Task-Row-Mapper und die Live-Score-Normalisierung. `backend/teaching/repo_live_queries.py` übernimmt seit Closeout v1.1 die Live-/Dashboard-Read-Model-Queries für Summary, Delta, Helper-Rows, Average-Scores und Live-Cursor. `backend/teaching/repo_material_queries.py` übernimmt seit Closeout v1.1 Material-CRUD, Upload-Intent, Finalize und Material-Reorder-SQL. `backend/teaching/repo_task_queries.py` übernimmt seit Closeout v1.1 Section-Task-CRUD, Task-Reorder, Course-Unit-Task-Read-Models und Latest-Submission-Aggregate. `backend/teaching/repo_unit_module_queries.py` übernimmt seit Closeout v1.1 Unit-Phasen, Unit-Module, Graph-Edges, Modul-Reorder, k-of-n-Unlock-Updates und den modularen Section-zu-Module-Helfer. `backend/teaching/repo_course_module_queries.py` übernimmt seit Closeout v1.1 Course-Module-CRUD, Course-Unit-Read-Modelle und Module-Section-Release-Queries. `backend/teaching/repo_section_queries.py` übernimmt seit Closeout v1.1 Unit-Section-CRUD und Section-Reorder. `backend/teaching/repo_unit_queries.py` übernimmt seit Closeout v1.1 Unit-Liste, Create, Update, Get/Delete und Existenzchecks. `backend/teaching/repo_member_queries.py` übernimmt seit Closeout v1.1 Course-Membership-Reads, Add/Remove, Student-Course-Checks und den Service-DSN-Delete-Fallback. `backend/teaching/repo_concern_box_queries.py` übernimmt seit Closeout v1.1 Concern-Box-Create, Listing, Archive und Restore. `backend/teaching/repo_ai_usage_queries.py` übernimmt seit Closeout v1.1 das owner-scoped AI-Usage-Read-Model. `backend/teaching/repo_db.py` ist dadurch von 4854 auf 1487 LOC gesunken. C2 ist als Hotspot-Reduktion deutlich fortgeschritten; verbliebene Course-Owner-Helfer sollten weiter beobachtet oder in einem späteren Course-Basis-Schnitt extrahiert werden.
...
- `backend/web/routes/learning_material_files.py` übernimmt seit Closeout v1.1 die learner-seitige Material-File-URL-Anreicherung, sichtbarkeitsgeprüfte Batch-Metadatenauflösung und modulare Material-Datei-Helfer. `backend/web/routes/learning.py` behält nur Kompatibilitätsaliase für bestehende Tests und Aufrufer.
- `backend/web/routes/learning_material_file_routes.py` besitzt seit Closeout v1.1 die kanonische Material-File-Download-Route und die legacy Section-Alias-Route selbst. Das Modul löst Repo-, Student-Guard-, Download- und Storage-Abhängigkeiten dynamisch über die Learning-Fassade auf, damit bestehende Test- und Runtime-Overrides kompatibel bleiben.
- `backend/web/routes/learning_upload_intents.py` besitzt seit Closeout v1.1 den studentischen Upload-Intent-Endpoint inklusive CSRF, Taskkind-MIME-Policy, Storage-Key-Erzeugung, Lazy-Storage-Wiring und optionaler Upload-Proxy-URL-Erzeugung. Das Modul löst Repo-, Config- und Storage-Abhängigkeiten dynamisch über die Learning-Fassade auf, damit bestehende Test- und Runtime-Overrides kompatibel bleiben.
- `backend/web/routes/learning_internal_upload_routes.py` besitzt seit Closeout v1.1 die internen Upload-Stub- und Upload-Proxy-Routen inklusive Same-Origin-Prüfung, URL-Host-Allowlisting, Upload-Limit und Proxy-Telemetrie. Das Modul löst Forwarder, Telemetrie, Config und Helper dynamisch über die Learning-Fassade auf.
- `backend/web/routes/learning_storage_validation.py` übernimmt seit Closeout v1.1 Storage-Integrity-Prüfung, lokale Validierungsbytes, begrenzten Presign-Download und Storage-Bytes-Laden für Submission-Validierung. `backend/web/routes/learning.py` behält Kompatibilitätsaliase für bestehende Tests und Monkeypatch-Punkte.
- `backend/web/routes/learning_submission_files.py` besitzt seit Closeout v1.1 Submission-History, Submission-File-Download, Submission-File-URLs und Download-Disposition-Normalisierung. Das Modul löst Repo, Use Cases, Download und Storage dynamisch über die Learning-Fassade auf.
- `backend/web/routes/learning.py` ist dadurch von 2884 auf 1670 LOC gesunken. C3 ist damit fortgeschritten, aber noch nicht abgeschlossen: Submission-Create/Finalize und die verbleibenden Submission-Validierungshelfer müssen weiter getrennt werden.
