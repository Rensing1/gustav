...
- `backend/web/routes/learning_downloads.py` übernimmt seit Closeout v1.1 den SSRF-geschützten, größenbegrenzten Download-Fetcher inklusive Public-to-Internal-Supabase-Rewrite. `backend/web/routes/learning.py` behält nur den alten privaten Wrapper für bestehende Monkeypatch-Punkte und ist dadurch von 2884 auf 2764 LOC gesunken. C3 ist damit begonnen, aber noch nicht abgeschlossen: Upload-Proxy, Upload-Intent, Submission- und Material-File-Flächen müssen weiter getrennt werden.
