...
- **Status 2025-11-09**: Upload-Proxy-Fallback-Test fixt Race, indem der FastAPI-Endpunkt selbst gepatcht wird (`backend/tests/test_learning_upload_proxy_fallback.py`). Async + Contract-Tests grün; Gesamt-Suite `AI_BACKEND=stub .venv/bin/pytest -q` → 717 passed / 10 skipped.  
- **Offen**: —  
...
- **Status 2025-11-09**: DSPy-Feedback-Programm liefert jetzt `FeedbackResult` inkl. `parse_status`; Local-Adapter loggt `parse_status`-Telemetrie und akzeptiert Backcompat-Resultate. Tests: `.venv/bin/pytest backend/tests/learning_adapters/test_feedback_program_dspy.py backend/tests/learning_adapters/test_feedback_program_dspy_parser.py backend/tests/learning_adapters/test_local_feedback_dspy.py -q`.  
...
- KomplettlauF: `AI_BACKEND=stub .venv/bin/pytest -q` → 717 passed, 10 skipped (Stand 2025-11-09).  
- DSPy: `.venv/bin/pytest backend/tests/learning_adapters/test_feedback_program_dspy.py backend/tests/learning_adapters/test_feedback_program_dspy_parser.py backend/tests/learning_adapters/test_local_feedback_dspy.py -q` grün.  
- **Bekannte Testabweichung**: —  
