...
> Historischer Hinweis (2026-07-17): Die hier beschriebene anbietergebundene Automatisierung wurde später vollständig entfernt. Verbindlich sind heute ausschließlich die lokalen Make-Ziele; aus diesem Dokument folgt keine aktuelle CI- oder Merge-Voraussetzung.
...
## MUST-FIX
- [API] Cookie‑Policy aktualisieren — Final: `SameSite=lax` (OAuth/OIDC‑kompatibel), host‑only in allen Umgebungen. OpenAPI und Tests sind entsprechend anzupassen.
...
## Tests
- Given dev env, When /auth/callback, Then Set-Cookie enthält `Secure; SameSite=lax` (host‑only) (backend/tests/test_auth_cookie_policies.py::test_callback_sets_host_only_cookie_dev).
...
## Freigabe
❌ Wegen API‑Must‑Fix (Cookie‑Policy). Nach Korrektur der OpenAPI (SameSite=lax; host‑only) und den kleinen Härtungen (SSRF/CI/Consistency) ist der PR freigabereif.

---

## Status-Update (applied 2025-11-12)
- DONE: OpenAPI Cookie-Doku angleichen (dev=prod: `Secure; SameSite=lax`, host‑only)
