...
0. **Robustheit: Worker-DML versteht Legacy-Queues nicht**  
   ...
   - Status Legacy-Table: `make import-legacy` (siehe `backend/tools/import_legacy_backup.py`) schreibt Submissions direkt nach `public.learning_submissions`; die Queue-Tabelle wird dabei nicht verwendet. Wir können `learning_submission_ocr_jobs` daher via Migration endgültig entfernen.  
