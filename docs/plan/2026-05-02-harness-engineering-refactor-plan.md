...
- Offen für C5: `backend/learning/repo_db.py` bleibt groß und enthält weiterhin Modular-Unit-Queries, Submission-Schreibfälle, Finalize-Logik und Worker-nahe Queue-/Extraction-Zugriffe. Die nächsten Schnitte sollen zuerst Modular-Unit-Read-Queries und danach Submission-Command-/Worker-Queries auslagern. Jeder Schnitt braucht einen Contract-Test für Modulbesitz plus die relevanten Learning-API-, Repo-Semantik-, Worker- und RLS/Migrationstests.
