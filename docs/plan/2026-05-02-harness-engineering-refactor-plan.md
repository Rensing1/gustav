...
- Offen für C4: `backend/web/routes/app.py` enthält weiterhin Diagnostik-Matrix, Live-Matrix, Live-Dashboard und Detail-Sheet-Read-Models. Die nächsten Schnitte sollen zuerst Diagnostik und danach Live-Read-Models in fokussierte Module verschieben. Jeder Schnitt braucht einen Contract-Test, der Modulbesitz, Router-Pfade oder Helper-Besitz schützt, plus die bestehenden Profil-, Session-, Header-, Route-Map- und API-Contract-Tests.
