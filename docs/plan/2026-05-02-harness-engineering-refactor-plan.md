...
- Offen für C4: `backend/web/routes/app.py` bleibt noch groß und enthält weiterhin unterschiedliche Browser-BFF-Verantwortungen. Als nächste Schnitte sollen Unit-Workspace-Read-Models und danach größere Dashboard-/Live-App-Read-Models in fokussierte Module wandern. Jeder Schnitt braucht einen Contract-Test, der Modulbesitz, Router-Pfade oder Helper-Besitz schützt, plus die bestehenden Profil-, Session-, Header-, Route-Map- und API-Contract-Tests.
