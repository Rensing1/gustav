...
- Done in working tree: `backend/web/routes/app_live_routes.py` besitzt Live-Matrix, Live-Course-Units, Live-Dashboard, Detail-Sheet und die zugehörigen Live-Read-Model-Helfer. `backend/web/routes/app.py` bleibt damit App-Router-Fassade für Session/BFF, Profil-Kompatibilität und Router-Komposition. Ein Contract-Test schützt Router- und Helper-Besitz außerhalb von `app.py`. `backend/web/routes/app.py` liegt nach diesem Schnitt bei 375 LOC. C4 ist damit als Hotspot-Reduktion abgeschlossen; zukünftiges Wachstum muss in den fokussierten App-Route-Modulen stattfinden.
...
- Offen für C5: `backend/learning/repo_db.py` enthält weiterhin Submission-Schreibfälle, Finalize-Logik, Submission-History-Reads und Worker-nahe Queue-/Extraction-Zugriffe. Die nächsten Schnitte sollen zuerst Submission-Command-/Finalize-Queries und danach Worker-/History-Queries auslagern. Jeder Schnitt braucht einen Contract-Test für Modulbesitz plus die relevanten Learning-API-, Repo-Semantik-, Worker- und RLS/Migrationstests.
