...
- Offen für C4: `backend/web/routes/app.py` enthält weiterhin Node-Editor, Diagnostik-Matrix, Live-Matrix, Live-Dashboard und Detail-Sheet-Read-Models. Die nächsten Schnitte sollen zuerst Node-Editor und danach Live-/Diagnostik-Read-Models in fokussierte Module verschieben. Jeder Schnitt braucht einen Contract-Test, der Modulbesitz, Router-Pfade oder Helper-Besitz schützt, plus die bestehenden Profil-, Session-, Header-, Route-Map- und API-Contract-Tests.
