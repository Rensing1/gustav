...
- Current check: PR-1 harness artifacts, repo-governed skill sources, `docs/plan/INDEX.md`, `docs/plan/MILESTONES.md`, `docs/plan/DECISIONS.md`, `make harness-minimum`, `make harness-signals`, `make test-import-boundaries`, `make test-frontend-h5p`, `make test-docker-image-smoke`, `make test-route-map`, `make quality-scorecard`, `make verify`, and `make test-full-prod-like` are present in the working tree.
...
- Local Make targets are the authoritative verification interface. Optional external automation may call them, but no provider-specific workflow is required for development or release.
...
- Warning hygiene: warning-only gates may exist as staged work, but green hard-gate commands should not emit incidental warnings. Any warning seen during a green gate is treated as cleanup work with evidence or a documented hardening item.
...
## Quality Gates
These gates are mandatory planning targets. The concrete implementation may use `make` targets, Python scripts, shell checks, or a combination, but every gate needs a clear local command and a clear pass/fail rule.
...
- Public Repo Hygiene Gate:
  ...
  - no `.env` leaks.
- Minimal Local Gate:
  - developers run the same repository-owned entry point on every supported host,
  - security baseline, public-repo hygiene, and image smoke are reachable through local Make targets,
  - verification must not use provider-specific or dev-only paths.
...
- Acceptance:
  ...
  - All mandatory verification is available through repository-owned local Make targets.
...
- Cleanup direction: continue migrating older tests away from local `sys.path` blocks and legacy alias paths whenever those tests are touched, then lower the `sys_path_mutations` baseline in the same reviewed change.
...
- Done in working tree: `backend/web/routes/learning_submission_files.py` besitzt Submission-History, Submission-File-Download, Submission-File-URLs und Download-Disposition-Normalisierung. Das Modul löst Repo, Use Cases, Download und Storage dynamisch über die Learning-Fassade auf, damit bestehende Monkeypatch- und Runtime-Overrides kompatibel bleiben. Ein Contract-Test schützt, dass diese Handler und Helfer nicht wieder in den Learning-Hotspot zurückwandern. `backend/web/routes/learning.py` liegt nach diesem Schnitt bei 1670 LOC.
- Done in working tree: `backend/web/routes/learning_submission_processing.py` übernimmt Submission-Payload-Validierung und den dev-only PDF-Processing-Helfer. `backend/web/routes/learning.py` behält Kompatibilitätsaliase für bestehende Tests und Monkeypatch-Punkte. Ein Contract-Test schützt, dass diese Helfer nicht wieder in den Learning-Hotspot zurückwandern. `backend/web/routes/learning.py` liegt nach diesem Schnitt bei 1473 LOC. C3 ist damit als Hotspot-Reduktion weitgehend abgeschlossen; verbleibende Create-/Finalize-Handler sollen bei weiterem Wachstum in ein eigenes Submission-Command-Modul wandern.
...
- Done in working tree: `backend/web/routes/app_profile_routes.py` besitzt die Profil-Read-/Write-Routen und CLI-Token-Routen. Das Modul löst User-, Identity-, CLI-Token- und Header-Abhängigkeiten dynamisch über die App-Fassade auf, damit bestehende Monkeypatch- und Runtime-Overrides kompatibel bleiben. Ein Contract-Test schützt, dass diese Handler nicht wieder in den App-Hotspot zurückwandern. `backend/web/routes/app.py` liegt nach diesem Schnitt bei 2230 LOC.
- Done in working tree: `backend/web/routes/app_learner_view_routes.py` besitzt Learner-Home, Learner-Concern-Box-View und Learner-Concern-Box-Create. Das Modul löst User-, Repo-, CSRF- und Header-Abhängigkeiten so auf, dass bestehende Monkeypatch- und Runtime-Overrides kompatibel bleiben. Ein Contract-Test schützt, dass diese Handler nicht wieder in den App-Hotspot zurückwandern. `backend/web/routes/app.py` liegt nach diesem Schnitt bei 2135 LOC.
- Done in working tree: `backend/web/routes/app_teacher_concern_routes.py` besitzt Teacher-Home, Teacher-Concern-Box-View sowie Archiv-/Restore-Routen für Kummerkasten-Einträge. Das Modul bündelt die inboxnahen Teacher-BFF-Routen, hält CSRF-/Owner-Prüfungen bei den Mutationen und bleibt über App-Fassaden-Aliase monkeypatch-kompatibel. Ein Contract-Test schützt, dass diese Handler und Helper nicht wieder in den App-Hotspot zurückwandern. `backend/web/routes/app.py` liegt nach diesem Schnitt bei 2007 LOC.
- Done in working tree: `backend/web/routes/app_teacher_course_routes.py` besitzt Teacher-Course-Liste, Course-Kontext und technische AI-Usage-Read-Models inklusive Course-/Member-/Usage-Helfer. Das Modul nutzt App-Fassaden-Aliase für gemeinsam verwendete Helper, damit Diagnostik, bestehende Monkeypatch-Tests und Runtime-Overrides kompatibel bleiben. Ein Contract-Test schützt, dass diese Handler und Helper nicht wieder in den App-Hotspot zurückwandern. `backend/web/routes/app.py` liegt nach diesem Schnitt bei 1640 LOC.
- Done in working tree: `backend/web/routes/app_teacher_unit_routes.py` besitzt Teacher-Units-Catalog und Teacher-Unit-Workspace inklusive Unit-Read-Model-Helfern, Course-Refs, Task-Id-/Submission-Pair-Helfern und Unit-Finder. `backend/web/routes/app.py` importiert Router, Endpoints und Helper nur noch als Kompatibilitätsfassade für Diagnostik-/Live-Flächen. Ein Contract-Test schützt Router- und Helper-Besitz außerhalb des App-Hotspots. `backend/web/routes/app.py` liegt nach diesem Schnitt bei 1145 LOC.
- Done in working tree: `backend/web/routes/app_teacher_node_editor_routes.py` besitzt den Teacher-Unit-Node-Editor-Read-Model-Endpoint für Section- und Module-Knoten. Das Modul nutzt die Unit-Read-Helfer aus `app_teacher_unit_routes.py` und hält Autorisierung, Node-Lookup, Materials- und Tasks-Formung aus dem App-Hotspot heraus. Ein Contract-Test schützt den Router-Besitz außerhalb von `app.py`. `backend/web/routes/app.py` liegt nach diesem Schnitt bei 1035 LOC.
- Done in working tree: `backend/web/routes/app_diagnostics_routes.py` besitzt Diagnostik-Course-Matrix, Learner-Profil und die zugehörigen Progress-Read-Model-Builder. Das Modul nutzt Course-/Unit-Helfer aus den fokussierten App-Route-Modulen und hält Diagnostik aus dem App-Hotspot heraus. Ein Contract-Test schützt Router- und Helper-Besitz außerhalb von `app.py`. `backend/web/routes/app.py` liegt nach diesem Schnitt bei 836 LOC.
- Done in working tree: `backend/web/routes/app_live_routes.py` besitzt Live-Matrix, Live-Course-Units, Live-Dashboard, Detail-Sheet und die zugehörigen Live-Read-Model-Helfer. `backend/web/routes/app.py` bleibt damit App-Router-Fassade für Session/BFF, Profil-Kompatibilität und Router-Komposition. Ein Contract-Test schützt Router- und Helper-Besitz außerhalb von `app.py`. `backend/web/routes/app.py` liegt nach diesem Schnitt bei 382 LOC. C4 ist damit als Hotspot-Reduktion abgeschlossen; zukünftiges Wachstum muss in den fokussierten App-Route-Modulen stattfinden.
...
- Done in working tree: `backend/learning/repo_course_unit_queries.py` übernimmt studentische Course-/Unit-Read-Queries für `list_courses_for_student` und `list_units_for_student_course`. `DBLearningRepo` bleibt öffentliche Fassade und delegiert mit DSN und Psycopg-Modul. Ein Contract-Test schützt, dass diese SQL-Signaturen nicht wieder in den Repo-Hotspot zurückwandern. `backend/learning/repo_db.py` liegt nach diesem Schnitt bei 2202 LOC.
- Done in working tree: `backend/learning/repo_submission_summary_queries.py` übernimmt die lightweight Latest-/Final-Submission-Summary pro Task für Learner-Taskkarten. `DBLearningRepo` delegiert weiterhin innerhalb der bestehenden Connection und übergibt die RLS-Kontext-Setter als Callbacks, damit Transaktions- und RLS-Verhalten unverändert bleiben. Ein Contract-Test schützt, dass die Summary-CTE nicht wieder in den Repo-Hotspot zurückwandert. `backend/learning/repo_db.py` liegt nach diesem Schnitt bei 2158 LOC.
- Done in working tree: `backend/learning/repo_modular_unit_queries.py` übernimmt Modular-Unit-Graph, Modul-Content, freigegebene Sections, Material-/Task-Reads, H5P-Content-Access und den modularen Section-Unlock-Helfer. `DBLearningRepo` bleibt öffentliche Fassade und delegiert diese Lesepfade plus den sicherheitsrelevanten Submission-Gate-Helfer weiter. Ein Contract-Test schützt, dass die modularen SQL-Helfer nicht wieder in den Repo-Hotspot zurückwandern. `backend/learning/repo_db.py` liegt nach diesem Schnitt bei 1510 LOC.
- Done in working tree: `backend/learning/repo_submission_command_queries.py` übernimmt Submission-Create, Feedback-Deduplizierung, Finalize-Latest-Feedback und Queue-Job-Erzeugung. `DBLearningRepo` bleibt öffentliche Fassade und übergibt Psycopg-, SQL-, JSON- und Pytest-Tagging-Abhängigkeiten explizit, damit RLS-Kontext, Row-Mapping und Queue-Table-Auflösung unverändert bleiben. Ein Contract-Test schützt, dass die SQL-Command-Blöcke nicht wieder in den Repo-Hotspot zurückwandern. `backend/learning/repo_db.py` liegt nach diesem Schnitt bei 681 LOC.
- Done in working tree: `backend/learning/repo_history_worker_queries.py` übernimmt Submission-History-Reads, `get_task_kind_for_student`, Queue-Table-Auflösung und `mark_extracted`. `DBLearningRepo` bleibt damit eine kleine Learning-Persistence-Fassade. Ein Contract-Test schützt, dass diese History-/Worker-Queries nicht wieder in den Repo-Hotspot zurückwandern. `backend/learning/repo_db.py` liegt nach diesem Schnitt bei 511 LOC. C5 ist damit als Hotspot-Reduktion abgeschlossen; zukünftige Query-Gruppen müssen in fokussierten Learning-Repo-Modulen wachsen.
...
- Vorgehen:
  ...
  - Redundante dokumentnahe Harness-Tests auf strukturierte Zustände umstellen, statt einzelne englische Marker pauschal zu verbieten.
- Akzeptanz:
...
- Done in working tree: `pyproject.toml` definiert eine zentrale Ruff-Konfiguration; `backend/requirements-harness.txt` installiert Ruff für lokale und CI-Harness-Gates, während `backend/web/requirements.txt` auf Runtime-Abhängigkeiten begrenzt bleibt.
...
- `.venv/bin/pytest -q backend/tests/test_visual_smoke_contract.py`: grün, 3 passed.
- `.venv/bin/pytest -q backend/tests/test_harness_minimum_contract.py::test_quality_scorecard_tracks_split_frontend_and_static_css_hotspots backend/tests/test_harness_minimum_contract.py::test_css_hotspot_docs_track_split_bundles_and_static_css_guardrails backend/tests/test_harness_minimum_contract.py::test_fastapi_static_gustav_css_references_are_documented_active_surfaces`: grün, 3 passed.
- `.venv/bin/pytest -q backend/tests/packaging/test_sveltekit_ui_shell_contract.py::test_design_system_styles_are_split_into_ordered_responsibility_bundles`: grün, 1 passed.
- `.venv/bin/pytest -q backend/tests/test_harness_minimum_contract.py backend/tests/test_visual_smoke_contract.py backend/tests/packaging/test_sveltekit_ui_shell_contract.py backend/tests/test_makefile_targets.py backend/tests/test_harness_test_strategy_docs_contract.py`: grün, 48 passed.
- `cd frontend && npm run check`: grün, 0 errors, 0 warnings.
- `cd frontend && npm test -- --run 'src/routes/learning/courses/[courseId]/units/[unitId]/page-contract.test.ts' src/lib/components/learning-unit/LearningTaskCard.test.ts src/lib/components/learning-unit/LearningUnitContentWorkspace.test.ts src/lib/components/learning-unit/LearningMaterialCard.test.ts src/lib/components/learning-unit/LearningGraphNode.contract.test.ts src/lib/components/ui/WorkspaceOutline.test.ts src/lib/components/ui/WorkspaceSettingsMenu.test.ts src/routes/account-menu-contract.test.ts`: grün, 8 files passed, 77 tests passed.
- `cd frontend && npm run test:e2e -- e2e/teacher-graph-module-actions.spec.ts`: grün, 1 Chromium Playwright test passed.
...
Für den Abschluss von Closeout v1.4 sind diese Befehle erfolgreich gelaufen:
...
- `make quality-scorecard`: grün nach Ausführung außerhalb der Sandbox, weil der enthaltene Docker-Image-Smoke Docker-Daemon-Zugriff braucht.
- `.venv/bin/pytest -q backend/tests/test_harness_minimum_contract.py::test_harness_refactor_plan_is_marked_complete_without_open_followups backend/tests/test_harness_minimum_contract.py::test_harness_documents_do_not_leave_open_refactor_followups`: grün, 2 passed.
- `make verify`: grün nach Abschluss von Closeout v1.4; umfasst DB-Preflight, Import-Boundary, OpenAPI, Architecture-Boundary, Route Map, DB-Test-Inventar, Supply Chain, Ruff/Pyflakes, Docker-Image-Smoke, Backend-Pytest mit 2014 passed/73 skipped, Svelte-Check mit 0 Fehlern/0 Warnungen, Frontend-Vitest mit 77 files/304 tests passed und H5P-Node mit 57 passed.
- `make test-visual-smoke`: grün, 5 Chromium-Smokes passed für Auth-Shells auf Desktop/Mobile, Teacher Workspace, Learner Workspace und H5P Workspace.
...
- PR 1 introduces provider-independent local Make targets for the security baseline, public-repo hygiene, and Docker/image smoke.
