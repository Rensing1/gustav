...
- Offen für C5: `backend/learning/repo_db.py` enthält weiterhin Submission-History-Reads, `get_task_kind_for_student`, Queue-Table-Auflösung und `mark_extracted`. Der nächste Schnitt soll diese History-/Worker-nahen Queries in ein fokussiertes Modul verschieben. Jeder Schnitt braucht einen Contract-Test für Modulbesitz plus die relevanten Learning-API-, Repo-Semantik-, Worker- und RLS/Migrationstests.
