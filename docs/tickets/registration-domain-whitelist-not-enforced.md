Title: Registrierung erlaubt fremde E-Mail-Domains trotz Schul-Domain-Restriktion

Status: Open

Problem:
- Erwartet ist, dass neue Konten nur mit `@school.example` angelegt werden können.
...
- Zusätzlich validiert die Keycloak-User-Profile-Konfiguration (`keycloak/realm-gustav.json`) nur das generelle E-Mail-Format. Es existiert kein Regex oder Script-Validator, der `@school.example` erzwingt.
...
Vorschlag:
1. **Keycloak erzwingen**: Im declarative user profile einen Regex-Validator für `email` ergänzen (z. B. `^.+@school\\.example$`) oder einen Custom-Validator verwenden. Dadurch schlägt jede Registrierung mit fremden Domains direkt im IdP fehl, auch wenn Nutzer die Registrierung-URL ohne die App aufrufen.
2. **App-Guardrail reaktivieren**: `ALLOWED_REGISTRATION_DOMAINS=@school.example` in `.env` setzen und eine kleine Vorschaltmaske oder HTMX-Form einführen, die vor dem Redirect die E-Mail sammelt und als `login_hint` an `/auth/register` übergibt. Dann greift die vorhandene 400-Validierung (`invalid_email_domain`) schon auf App-Ebene.
