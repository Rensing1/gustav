...
import { handleAuthCallback, startLoginFlow } from "./backend-auth";
...
function decodeFlowCookie(rawCookie: string): { state: string; nonce: string } {
  const [payload] = rawCookie.split(".", 1);
  return JSON.parse(Buffer.from(payload, "base64url").toString("utf-8")) as { state: string; nonce: string };
}
