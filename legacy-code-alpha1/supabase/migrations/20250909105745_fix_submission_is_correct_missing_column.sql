-- Fix missing is_correct column in submission table and update RPC functions
-- Problem: _get_submission_status_matrix_uncached references sub.is_correct which doesn't exist

BEGIN;

-- Add missing is_correct column to submission table
ALTER TABLE submission 
ADD COLUMN IF NOT EXISTS is_correct BOOLEAN;

-- Update the function to handle null is_correct values properly
    ...
    submission_status AS (
        SELECT 
            ...
            jsonb_build_object(
                ...
                'is_correct', (
                    SELECT COALESCE(BOOL_OR(sub.is_correct), FALSE)
                    FROM submission sub 
                    WHERE sub.student_id = es.student_id 
                    AND sub.task_id = ut.task_id
                ),
                ...
                'has_feedback', EXISTS(
                    ...
                    WHERE sub.student_id = es.student_id 
                    AND sub.task_id = ut.task_id
                    AND (sub.ai_feedback IS NOT NULL OR sub.teacher_override_feedback IS NOT NULL)
                )
            ) as status_data,
