-- Fix: Update functions to match actual schema
-- 1. Fix learning_unit references: created_by -> creator_id
-- 2. Fix course_learning_unit_assignment: learning_unit_id -> unit_id, remove order_in_course
-- 3. Fix remaining course references: created_by -> creator_id

-- First, fix the remaining course created_by -> creator_id references in batch5 functions

-- Fix get_submissions_for_course_and_unit (line 161)
...
-- Fix _get_submission_status_matrix_uncached (line 234)
...
-- Fix get_section_statuses_for_unit_in_course (line 389)
...
-- Fix publish_section_for_course (line 446 & 455)
...
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1 
        FROM unit_section s
        JOIN course_learning_unit_assignment cua ON cua.unit_id = s.learning_unit_id  -- Fixed: changed learning_unit_id to unit_id
...
-- Fix unpublish_section_for_course (line 514)
DROP FUNCTION IF EXISTS public.unpublish_section_for_course(TEXT, UUID, UUID);
...
-- Fix create_section (lines 559 & 622)
...
-- Fix update_section_materials (line 622)
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        WHERE s.id = p_section_id AND lu.creator_id = v_user_id  -- Fixed: changed created_by to creator_id
    ) THEN
        RAISE EXCEPTION 'Unauthorized: Teacher does not own the learning unit';
    END IF;
...
-- Fix update_learning_unit and delete_learning_unit from batch2
-- Update update_learning_unit (line 413)
DROP FUNCTION IF EXISTS public.update_learning_unit(TEXT, UUID, TEXT);
...
-- Update delete_learning_unit (line 455)
DROP FUNCTION IF EXISTS public.delete_learning_unit(TEXT, UUID);
...
-- Finally, fix assign_unit_to_course to handle the correct schema
...
-- Also fix unassign_unit_from_course for consistency
