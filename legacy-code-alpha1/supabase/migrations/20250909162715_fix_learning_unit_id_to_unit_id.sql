-- Fix all references from s.learning_unit_id to s.unit_id in unit_section table
-- The column was renamed but many functions still use the old name
...
BEGIN
    ...
    -- Verify section belongs to a unit assigned to this course (FIX: learning_unit_id -> unit_id)
    IF NOT EXISTS (
        SELECT 1 
        FROM unit_section s
        JOIN course_learning_unit_assignment cua ON cua.learning_unit_id = s.unit_id
