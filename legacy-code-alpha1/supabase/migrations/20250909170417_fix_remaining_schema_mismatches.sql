-- Fix remaining schema mismatches identified in test run
-- Issues addressed:
-- 1. p.name -> p.full_name in profiles table
-- 2. cua.learning_unit_id -> cua.unit_id in course_learning_unit_assignment table
-- 3. Remaining s.learning_unit_id references

-- Drop and recreate functions that still have schema issues
...
-- First, fix the already fixed function that has p.name issue
DROP FUNCTION IF EXISTS public.get_submissions_for_course_and_unit(TEXT, UUID, UUID);

-- Fix get_submissions_for_course_and_unit with correct profiles column name
...
BEGIN
    ...
    SELECT 
        sub.student_id,
        p.full_name as student_name,  -- FIXED: was p.name
        sub.task_id,
    ...
    ORDER BY p.full_name, t.title, sub.submitted_at DESC;  -- FIXED: was p.name
END;
$$;

-- Recreate create_submission with all fixes
...
BEGIN
    ...
    -- Get course_id for this task (FIXED: all schema references)
...
-- Fix get_mastery_tasks_for_course
...
-- Fix get_submission_status_matrix with profiles.full_name
...
-- Fix feedback table by adding missing columns that are referenced in the code
ALTER TABLE feedback 
ADD COLUMN IF NOT EXISTS feedback_text TEXT;
-- Note: page_identifier was already added in previous migration