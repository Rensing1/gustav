-- Fix teacher_feedback to teacher_override_feedback in _get_submission_status_matrix_uncached
        ...
        SELECT 
            ...
            jsonb_build_object(
                ...
                'has_feedback', EXISTS(
                    ...
                    WHERE sub.student_id = es.student_id 
                    AND sub.task_id = ut.task_id
                    AND (sub.ai_feedback IS NOT NULL OR sub.teacher_override_feedback IS NOT NULL)  -- Fixed column name
                )
            ) as status_data,
