-- Fix column reference error in get_published_section_details_for_student
-- The function tries to reference rt.id which doesn't exist
-- It should use t.order_in_section from task_base table

-- Drop the existing 2-parameter function and create a corrected version
...
BEGIN
    ...
    -- Get the unit_id from course_unit_assignment for this course
    SELECT unit_id INTO v_unit_id
    FROM course_unit_assignment
...
-- Also fix the 4-parameter version if it still exists
    ...
    task_details AS (
        ...
        SELECT 
            ...
            jsonb_build_object(
                ...
                'title', t.title,
                'task_type', t.task_type,
                'order_in_section', t.order_in_section,  -- Fixed: Use t.order_in_section instead of rt.order_in_section
                'is_mastery', FALSE,
                'max_attempts', COALESCE(rt.max_attempts, 1),
                'prompt', COALESCE(rt.prompt, ''),
                'submission_count', COUNT(sub.id),
