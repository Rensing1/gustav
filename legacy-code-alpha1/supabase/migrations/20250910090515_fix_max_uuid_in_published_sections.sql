...
-- Use array aggregation to get the latest submission ID
            ...
            jsonb_build_object(
                ...
                CASE 
                    WHEN COUNT(sub.id) > 0 THEN
                        jsonb_build_object(
                            'id', (array_agg(sub.id ORDER BY sub.submitted_at DESC))[1]::TEXT,
                            'is_correct', BOOL_OR(sub.is_correct),
                            'submitted_at', MAX(sub.submitted_at),
                            'has_feedback', BOOL_OR(sub.ai_feedback IS NOT NULL OR sub.teacher_override_feedback IS NOT NULL),
                            'feedback_viewed', BOOL_OR(sub.feedback_viewed_at IS NOT NULL)
                        )
                    ELSE NULL
                END
            ) as task_data
