...
-- The function references lu.created_by but the column is lu.creator_id
...
BEGIN
    ...
    -- FIXED: Changed lu.created_by to lu.creator_id
    IF NOT EXISTS (
        SELECT 1 
        FROM task_base t
        JOIN unit_section s ON s.id = t.section_id
        JOIN learning_unit lu ON lu.id = s.unit_id
        WHERE t.id = p_task_id AND lu.creator_id = v_user_id  -- Fixed: was lu.created_by
    ) THEN
