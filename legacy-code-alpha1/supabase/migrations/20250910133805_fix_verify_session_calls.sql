-- Fix verify_session calls that don't exist
-- Replace with proper validate_session_and_get_user function
...
-- Comment on changes
COMMENT ON FUNCTION public.get_next_due_mastery_task(TEXT, UUID, UUID) IS 'Fixed: Uses validate_session_and_get_user instead of non-existent verify_session';
COMMENT ON FUNCTION public.get_tasks_for_section(TEXT, UUID) IS 'Fixed: Uses validate_session_and_get_user instead of non-existent verify_session';
COMMENT ON FUNCTION public.get_mastery_tasks_for_section(TEXT, UUID) IS 'Fixed: Uses validate_session_and_get_user instead of non-existent verify_session';
COMMENT ON FUNCTION public.get_section_tasks(TEXT, UUID, UUID) IS 'Fixed: Uses validate_session_and_get_user instead of non-existent verify_session';
COMMENT ON FUNCTION public.get_task_details(TEXT, UUID) IS 'Fixed: Uses validate_session_and_get_user instead of non-existent verify_session';