-- Fix create_submission function to use correct session table (auth_sessions instead of user_sessions)
-- The previous migration used the wrong session table name
...
COMMENT ON FUNCTION create_submission IS 'Creates a new submission with proper mastery task handling and correct session validation using auth_sessions table. Regular tasks respect max_attempts limits, mastery tasks have unlimited attempts for spaced repetition learning.';