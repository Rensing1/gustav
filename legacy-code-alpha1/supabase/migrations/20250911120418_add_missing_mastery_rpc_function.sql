-- Add missing RPC function: get_next_mastery_task_or_unviewed_feedback
-- This function is critical for the Wissensfestiger (Knowledge Reinforcement) system
-- It first checks for unviewed feedback, then returns the next due mastery task
...
BEGIN
    ...
    -- If unviewed feedback exists, return it with task details
...
-- Add helpful comment
COMMENT ON FUNCTION public.get_next_mastery_task_or_unviewed_feedback IS 'Returns either unviewed feedback (priority) or the next due mastery task for a student in a course. Critical for Wissensfestiger system.';