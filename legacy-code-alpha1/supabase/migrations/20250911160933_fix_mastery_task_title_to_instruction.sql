-- Fix task_base.title references (column doesn't exist, use instruction instead)
-- The task_base table has instruction, not title
...
-- Update comment
COMMENT ON FUNCTION public.get_next_mastery_task_or_unviewed_feedback IS 'Returns either unviewed feedback (priority) or the next due mastery task for a student in a course. Fixed to use instruction instead of non-existent title column.';