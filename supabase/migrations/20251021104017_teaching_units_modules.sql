...
create index if not exists idx_units_author on public.units(author_id);
...
grant select, insert, update, delete on public.units to gustav_limited;
grant select, insert, update, delete on public.course_modules to gustav_limited;

do $$ begin
  if exists (select 1 from pg_policies where schemaname='public' and tablename='units') then
    drop policy if exists units_select_author on public.units;
    drop policy if exists units_insert_author on public.units;
    drop policy if exists units_update_author on public.units;
    drop policy if exists units_delete_author on public.units;
  end if;
