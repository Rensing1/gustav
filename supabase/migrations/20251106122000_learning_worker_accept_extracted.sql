-- Allow worker helper functions to operate on submissions in 'pending' or 'extracted'
...
begin
    update public.learning_submissions
       set analysis_status = 'completed',
           ...
           analysis_json = p_analysis_json::jsonb,
           error_code = null,
           completed_at = now(),
    ...
    if not found then
        raise exception 'learning_worker_update_completed: submission must be pending or extracted';
    end if;
end;
$$;
...
begin
    ...
    if not found then
        raise exception 'learning_worker_update_failed: submission must be pending or extracted';
    end if;
end;
$$;
...
begin
    ...
    if not found then
        raise exception 'learning_worker_mark_retry: submission not pending or extracted';
    end if;
end;
$$;

grant execute on function public.learning_worker_update_completed(uuid, text, text, json) to gustav_worker;
