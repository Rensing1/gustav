-- Make learning worker helper functions tolerant to status drift in tests/dev
-- Do not raise when a submission is no longer in the expected intermediate state.
-- Instead, perform no-op and return. This reduces flakiness when suites overlap
-- worker phases or when retries race with status transitions.
