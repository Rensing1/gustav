-- Teaching Live matrix: expose H5P completion as a boolean flag.
--
-- Why:
--   The teacher Live-Unterrichts-Matrix should distinguish three states for H5P tasks:
--     - no submission (—)
--     - attempted/bearbeitet (•)
--     - completed/abgeschlossen (✓)
--
--   We define "completed" for H5P as: there exists at least one H5P submission
--   with full score (score_raw == score_max and score_max > 0) for the student+task.
--   This flag is intentionally derived from submission history (not just the latest).
--
-- Security:
--   This helper is SECURITY DEFINER and checks course ownership via p_owner_sub.
--   It can be executed by the limited application role `gustav_limited`.
