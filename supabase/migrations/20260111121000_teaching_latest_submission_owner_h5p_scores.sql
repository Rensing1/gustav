-- Teaching: include H5P score fields in latest-submission helper (owner scope)
--
-- Why:
--   Teacher Live detail needs to render a read-only H5P review player and show
--   the student's latest score (raw/max). Teachers cannot read
--   `learning_submissions` directly due to RLS, therefore the SECURITY DEFINER
--   helper must expose `score_raw/score_max` for `kind='h5p'`.
--
-- Local = Prod: schema changes only via migrations.

set check_function_bodies = off;

-- When changing OUT parameters of a SQL function, we must drop the old
-- version first; CREATE OR REPLACE cannot change the row type.
drop function if exists public.get_latest_submission_for_owner(text, uuid, uuid, uuid, text);
