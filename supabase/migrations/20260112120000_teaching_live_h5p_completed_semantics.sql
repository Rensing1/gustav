-- Teaching Live: H5P "completed" semantics (Option B).
--
-- Why:
--   Some H5P contents report "unscored" results where score_max == 0.
--   Product decision (Option B): treat a submission as completed when
--   score_raw == score_max, including the 0/0 case.
--
-- Security:
--   This helper is SECURITY DEFINER and checks course ownership via
--   `app.current_sub`. It can be executed by the limited application role
--   `gustav_limited`.
