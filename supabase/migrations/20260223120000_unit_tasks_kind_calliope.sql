-- Teaching/Learning: extend unit_tasks.kind enum check to include Calliope tasks.
--
-- Why:
--   Calliope tasks use Task.kind='calliope' and require MakeCode HEX upload-only submissions.
--
-- Notes:
...
alter table public.unit_tasks
  ...
  check (kind in ('native', 'h5p', 'visual', 'scratch', 'calliope'));
