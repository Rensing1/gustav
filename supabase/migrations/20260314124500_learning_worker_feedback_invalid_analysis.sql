-- Accept deterministic structured-analysis failures without retrying the worker.
    ...
    update public.learning_submissions
       set analysis_status = 'failed',
           ...
           feedback_last_error = case
               when p_error_code in ('feedback_failed', 'feedback_invalid_analysis') then trimmed
               else feedback_last_error
           end,
           feedback_last_attempt_at = case
               when p_error_code in ('feedback_failed', 'feedback_invalid_analysis') then now()
